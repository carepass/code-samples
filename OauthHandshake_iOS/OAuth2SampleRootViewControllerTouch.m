...
static NSString *const kCarePassSyncAppServiceName = @"OAuth Sample: CarePass Sync";
static NSString *const kCarePassSyncServiceName = @"CarePassSync";
...
- (GTMOAuth2Authentication *)authForCarePassSync;
...
static NSString *const kCarePassSyncAPIKey         = @"CarePassSyncAPI";
static NSString *const kCarePassSyncClientIDKey     = @"CarePassSyncClientID";
static NSString *const kCarePassSyncClientSecretKey = @"CarePassSyncClientSecret";
...
- (void)awakeFromNib {
    ...
    if (auth.canAuthorize) {
        ...
        //
        // Perhaps we have a saved authorization for CarePass Sync instead; try getting
        // that from the keychain
        
        clientID = [defaults stringForKey:kCarePassSyncClientIDKey];
        clientSecret = [defaults stringForKey:kCarePassSyncClientSecretKey];
        
        if (clientID && clientSecret) {
            auth = [self authForCarePassSync];
            if (auth) {
                auth.clientID = clientID;
                auth.clientSecret = clientSecret;
                
                BOOL didAuth = [GTMOAuth2ViewControllerTouch authorizeFromKeychainForName:kCarePassSyncAppServiceName
                                                                           authentication:auth];
                if (didAuth) {
                    // select the CarePass Sync radio button
...
- (void)signOut {
    ...
    // remove the stored CarePassSync authentication from the keychain, if any
    [GTMOAuth2ViewControllerTouch removeAuthFromKeychainForName:kCarePassSyncAppServiceName];
...
- (GTMOAuth2Authentication *)authForCarePassSync {
    ...
    NSString *url = @"https://www.carepass.com/carepass/oauth/token";
    ...
    auth = [GTMOAuth2Authentication authenticationWithServiceProvider:kCarePassSyncServiceName
                                                             tokenURL:tokenURL
...
- (void)signInToCarePassSync {
    [self signOut];
    
    GTMOAuth2Authentication *auth = [self authForCarePassSync];
    auth.scope = @"IDENTITY,INSURANCE,FITNESS,LIFESTYLE";
    ...
    NSURL *authURL = [NSURL URLWithString:@"https://www.carepass.com/carepass/oauth/authorize"];
    ...
    // in
    viewController.browserCookiesURL = [NSURL URLWithString:@"https://api.carepass.com"];
...
- (void)doAnAuthenticatedAPIFetch {
    ...
    if ([self isGoogleSegmentSelected]) {
        ...
        // CarePass current user identity
        urlStr = @"https://api.carepass.com/user-directory-api/users/currentUser";
    }
...
- (void)updateUI {
    ...
    if ([self isSignedIn]) {
        ...
        NSLog(@"accessToken: %@", self.auth.accessToken);
...
- (void)saveClientIDValues {
    ...
    if ([self isGoogleSegmentSelected]) {
        ...
        [defaults setObject:clientID forKey:kCarePassSyncClientIDKey];
        [defaults setObject:clientSecret forKey:kCarePassSyncClientSecretKey];
    }
}
...
- (void)loadClientIDValues {
    ...
    if ([self isGoogleSegmentSelected]) {
        ...
        self.clientIDField.text = [defaults stringForKey:kCarePassSyncClientIDKey];
        self.clientSecretField.text = [defaults stringForKey:kCarePassSyncClientSecretKey];
    }
}
